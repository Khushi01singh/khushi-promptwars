...
class JobMatcher:
    ...
    def _evaluate_node(self, state: JobMatchState) -> JobMatchState:
        ...
        # --- SECURELY FETCH API KEYS FROM ENVIRONMENT ---
        # Absolutely no keys are printed or masked in logs
        google_api_key = os.getenv("GOOGLE_API_KEY")
        openai_api_key = os.getenv("OPENAI_API_KEY")
        
        if not google_api_key and not openai_api_key:
            logger.error("DEBUG: Both GOOGLE_API_KEY and OPENAI_API_KEY are missing from the environment!")
        else:
            logger.info("DEBUG: API Key securely detected from environment.")
        # ------------------------------------------------

        try:
            # Dynamically initialize the correct LLM based on model name
            if self.model_name.startswith("gpt-"):
                from langchain_openai import ChatOpenAI
                if not openai_api_key:
                    raise ValueError("OPENAI_API_KEY is required for GPT models but was not found.")
                llm = ChatOpenAI(
                ...
                if not google_api_key:
                    raise ValueError("GOOGLE_API_KEY is required for Gemini models but was not found.")
